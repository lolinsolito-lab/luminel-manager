...
const planChunk = getChunk('const PLAN_STEPS = [');
...
// It goes from const PAIN_POINTS = [ to the end of ProblemSection which is before const PLAN_STEPS = [
newLanding = newLanding.replace(
...
// It goes from const PLAN_STEPS = [ to the end of PlanSection which is before const FAILURE_STAGES = [
newLanding = newLanding.replace(
  /const PLAN_STEPS = \[.*?const FAILURE_STAGES = \[/s,
  planChunk + '\n\nconst FAILURE_STAGES = ['
);

// 4. FAILURE_STAGES and FailureSection
// It goes from const FAILURE_STAGES = [ to the end of FailureSection which is before const LeadMagnetSection: React.FC
newLanding = newLanding.replace(
...
// 5. EcosystemSection
// Insert before const MetricsGridSection: React.FC
newLanding = newLanding.replace(
  /const MetricsGridSection: React\.FC = \(\) => \{/s,
  ecosystemChunk + '\n\nconst MetricsGridSection: React.FC = () => {'
);

// 6. AiCoachHeroSection and remove EcosystemSection from render
// Find id="soluzioni" block and replace it
newLanding = newLanding.replace(
